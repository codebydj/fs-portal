...
exports.submitSelection = async (req, res) => {
  ...
  if (!selections || !Array.isArray(selections) || selections.length === 0) {
    return res.status(400).json({ error: "Selections are required", code: "INVALID_REQUEST" });
  }
  ...
  try {
    ...
    if (!configSnap.exists) {
      return res.status(403).json({ error: "Selection configuration not found", code: "SELECTION_CLOSED" });
    }

    const config = configSnap.data();
    const now = Timestamp.now();

    if (!config.selection_open) {
      return res.status(403).json({ error: "Faculty selection is currently closed", code: "SELECTION_CLOSED" });
    }

    if (config.end_time && config.end_time.toMillis() < now.toMillis()) {
      return res.status(403).json({ error: "Faculty selection window has expired", code: "SELECTION_CLOSED" });
    }
    ...
    if (allSubjectIds.length !== selectedSubjectIds.length) {
      return res.status(400).json({ error: "You must select faculty for all subjects", code: "INCOMPLETE_SELECTION" });
    }

    const missingSubjects = allSubjectIds.filter((id) => !selectedSubjectIds.includes(id));
    ...
    if (uniqueSubjects.size !== selectedSubjectIds.length) {
      return res.status(400).json({ error: "Duplicate subject selections found", code: "INVALID_REQUEST" });
    }
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (studentSnap.data().has_submitted) {
        throw new PortalError("ALREADY_SUBMITTED", "You have already submitted your faculty selection.");
      }

      // 3. Read all faculty docs and validate seats
      const facultyRefs = selections.map((s) => db.collection("faculty").doc(s.faculty_id));
      const facultySnaps = await Promise.all(facultyRefs.map((ref) => transaction.get(ref)));
      ...
      for (let i = 0; i < facultySnaps.length; i++) {
        ...
        if (!snap.exists) {
          throw new PortalError("NOT_FOUND", `Faculty not found: ${sel.faculty_id}`);
        }
        ...
        if (faculty.current_count >= faculty.max_limit) {
          throw new PortalError("SEATS_FULL", `No seats available for ${faculty.name}`);
        }
      }
    ...
    return res.status(200).json({ success: true, message: "Faculty selection submitted successfully" });
  } catch (err) {
