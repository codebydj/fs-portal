      ...
      try {
        const results = await Promise.all(
          selections.map(async (sel) => {
            ...
            return {
              subject: subjectSnap.exists() ? subjectSnap.data() : { name: sel.subject_id, code: "—" },
              faculty: facultySnap.exists() ? facultySnap.data() : { name: sel.faculty_id },
            };
          })
        );
        setEnriched(results);
      } catch { /* show raw IDs if fetch fails */ }
  ...
  return (
    ...
    >
      <div className="px-5 py-4 border-b border-slate-100 bg-slate-50">
        <h3 className="font-semibold text-slate-900 font-display text-sm">Your Faculty Selections</h3>
      </div>
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-100">
            ...
            <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase">Faculty Selected</th>
          </tr>
        </thead>
        <tbody>
          {enriched.map((row, i) => (
            <tr key={i} className="border-b border-slate-100 last:border-0">
              <td className="px-5 py-3 font-medium text-slate-800">{row.subject.name}</td>
              <td className="px-5 py-3 font-mono text-xs text-slate-500">{row.subject.code}</td>
...
export default function StudentDashboard() {
  ...
  const isSelectionClosed = configLoaded && (selectionOpen === false || expired);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const payload = Object.entries(selections).map(([subject_id, faculty_id]) => ({
        subject_id,
        faculty_id,
      }));
      await submitSelection(payload);
      // Save submitted selections for display
      const selectionData = payload.map(({ subject_id, faculty_id }) => ({ subject_id, faculty_id, pin: user.pin }));
      localStorage.setItem("previous_selections", JSON.stringify(selectionData));
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-0.5 flex-wrap">
              <span className="text-sm text-slate-500">
                <span className="font-mono text-xs bg-slate-100 px-2 py-0.5 rounded">{user?.pin}</span>
              </span>
        ...
        {isSubmitted && (
          ...
          >
            ...
            <div>
              <p className="font-medium text-green-800">You have already submitted your faculty selection.</p>
              <p className="text-sm text-green-600 mt-0.5">Your selections are locked and cannot be changed.</p>
            </div>
          </motion.div>
        )}
        ...
        {isSelectionClosed && !isSubmitted && (
          ...
          >
            ...
            <div>
              <p className="font-medium text-red-800">Faculty selection window is closed.</p>
              <p className="text-sm text-red-600 mt-0.5">Please contact your administrator for assistance.</p>
            </div>
          </motion.div>
        )}
        ...
        {!isSubmitted && (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-semibold text-primary-600">{selectedCount} / {totalSubjects} subjects</span>
            </div>
            <div className="h-2 bg-slate-200 rounded-full overflow-hidden">
              <motion.div
                className="h-full bg-primary-600 rounded-full"
                animate={{ width: totalSubjects > 0 ? `${(selectedCount / totalSubjects) * 100}%` : "0%" }}
